...
Demo.Methods.displayFileItemHTML = function(content) {
  return (
      ...
      'href="#" style="display: block;" download="' +
      content.name +
      ...
      (content.direction === Demo.Skylink.DATA_TRANSFER_TYPE.DOWNLOAD
          ? '<a class="btn btn-default cancel c-' +
          content.peerId +
          '" style="margin-top: 15px; border-color: #d9534f; color: #d9534f;" ' +
...
Demo.Methods.displayChatItemHTML = function(
    peerId,
    timestamp,
    content,
    isPrivate
  ...
  if (timestamp.getHours() < 10) Hours = "0" + timestamp.getHours();
  else Hours = timestamp.getHours();
  if (timestamp.getMinutes() < 10) Minutes = "0" + timestamp.getMinutes();
  else Minutes = timestamp.getMinutes();
  if (timestamp.getSeconds() < 10) Seconds = "0" + timestamp.getSeconds();
  else Seconds = timestamp.getSeconds();

  return (
      "<div " +
      (typeof isPrivate === "string" ? 'id="file-' + isPrivate + '"' : "") +
...
Demo.Methods.displayRecordingSessionHTML = function(recordingId) {
  ...
  $("#recording_log").append(
      ...
      '<p class="list-group-item-heading"><b><small><span id="recording_' +
      recordingId +
      ...
      '<small><b>STATUS:</b>&nbsp;&nbsp;<em id="recording_' +
      recordingId +
      '_state" style="font-style:normal;">STARTED</em></small></blockquote>' +
      '<p id="recording_' +
      recordingId +
      '_btn"></p>' +
      '<div id="recording_' +
...
Demo.Methods.displayChatMessage = function(peerId, content, isPrivate) {
  ...
  $(element).append(
      Demo.Methods.displayChatItemHTML(peerId, timestamp, content, isPrivate)
  );
 ...
 *********************************************************/
//---------------------------------------------------
Demo.Skylink.on("incomingData", function(
    data,
    transferId,
    peerId,
    transferInfo,
...
Demo.Skylink.on("incomingDataRequest", function(
    transferId,
    peerId,
    transferInfo,
    isSelf
...
Demo.Skylink.on("dataTransferState", function(
    state,
    transferId,
    peerId,
    transferInfo,
  ...
  switch (state) {
    ...
    case Demo.Skylink.DATA_TRANSFER_STATE.UPLOAD_STARTED:
      if (
          document.getElementById("file-" + transferId) &&
          $("#" + transferId + " ." + peerId).length === 0
      ) {
        $("#" + transferId + " ." + peerId).append(
            '<tbody class="' + peerId + '"></tbody>'
        );
        return;
      }
    ...
    case Demo.Skylink.DATA_TRANSFER_STATE.DOWNLOAD_STARTED:
      if (
        ...
        if (transferInfo.data) {
          if (window.webrtcDetectedBrowser !== "IE") {
            $("#" + transferId + "_btn").attr(
                "href",
                URL.createObjectURL(transferInfo.data)
            );
          } else {
      ...
      var displayName = Demo.Skylink.getPeerInfo(transferInfo.senderPeerId)
          .userData;
      transferInfo.transferId = transferId;
    ...
    case Demo.Skylink.DATA_TRANSFER_STATE.UPLOADING:
      ...
      if (
        ...
        $("#" + transferId).append(
            ...
            peerId +
            ' cancel" ' +
            'style="color: #d9534f;" onclick="cancelTransfer(\'' +
            peerId +
            "', '" +
            transferId +
            "');\">" +
    ...
    case Demo.Skylink.DATA_TRANSFER_STATE.UPLOAD_COMPLETED:
      ...
      Demo.Methods.displayChatMessage(
          displayName,
          "File received: " + transferInfo.name
      );
      $("#" + transferId)
          .find("." + peerId)
          .html("&#10003;");
      $("#" + transferId + " .c-" + peerId + ".cancel").css("opacity", 0.5);
      $("#" + transferId + " .c-" + peerId + ".cancel").css(
          "cursor",
          "not-allowed"
      );
    ...
    case Demo.Skylink.DATA_TRANSFER_STATE.DOWNLOAD_COMPLETED:
      ...
      $("#file-" + transferId + " .c-" + peerId + ".cancel").css(
          "opacity",
          0.5
      );
      $("#file-" + transferId + " .c-" + peerId + ".cancel").css(
...
Demo.Skylink.on("incomingMessage", function(message, peerId, peerInfo, isSelf) {
  Demo.Methods.displayChatMessage(
      isSelf ? "You" : peerInfo.userData,
      (message.isDataChannel ? "P2P" : "Socket") +
      " -> " +
...
Demo.Skylink.on("peerJoined", function(peerId, peerInfo, isSelf) {
  ...
  $(".videocamera").on("click", ".btn", function() {
    $(this)
        .addClass("active-btn")
        .siblings()
        .removeClass("active-btn");
  ...
  $(".screenshare").on("click", ".btn", function() {
    $(this)
        .addClass("active-btn")
        .siblings()
        .removeClass("active-btn");
  ...
  $(".recording").on("click", ".btn", function() {
    $(this)
        .addClass("active-btn")
        .siblings()
        .removeClass("active-btn");
  });
  $(".stream").on("click", ".btn", function() {
    $(this)
        .addClass("active-btn")
        .siblings()
        .removeClass("active-btn");
  ...
  if (isSelf) {
    ...
    var newListEntry =
        ...
        peerInfo.userData +
        "</span><br>" +
        '<input class="select-user" target="' +
        peerId +
        '" type="checkbox" onclick="selectTargetPeer(this);"></td><td>';
    var titleList = [
      "Joined Room",
      "Handshake: Welcome",
      "Handshake: Offer",
  ...
  if ($("#video" + peerId).length === 0) {
    ...
    peerElm.className = "col-md-4 peervideo";
    ...
    $(peerElm).append(
        '<div class="connstats-wrapper"><button class="toggle-connstats" data="' +
        (isSelf ? "MCU" : peerId) +
        '">See ' +
        (isSelf ? "MCU " : "") +
        'Stats</button><div class="row connstats">' +
...
Demo.Skylink.on("mediaAccessSuccess", function(stream) {
  Demo.Methods.displayChatMessage(
      "System",
      "Audio and video access is allowed."
  );
});
//---------------------------------------------------
Demo.Skylink.on("mediaAccessError", function(error) {
  ...
  Demo.Methods.displayChatMessage(
      "System",
      "Failed to join room as video and audio stream is required."
  );
});
//---------------------------------------------------
...
Demo.Skylink.on("readyStateChange", function(state, error) {
  if (state === Demo.Skylink.READY_STATE_CHANGE.ERROR) {
    for (var errorCode in Demo.Skylink.READY_STATE_CHANGE_ERROR) {
      if (
          Demo.Skylink.READY_STATE_CHANGE_ERROR[errorCode] === error.errorCode
      ) {
        alert(
            "An error occurred parsing and retrieving server code.\n" +
            "Error was: " +
            errorCode
        );
        break;
...
Demo.Skylink.on("iceConnectionState", function(state, peerId) {
  ...
  switch (state) {
    ...
    case Demo.Skylink.ICE_CONNECTION_STATE.COMPLETED:
      color = "green";
      $(
          "#video" + (peerId === "MCU" ? _peerId : peerId) + " .connstats-wrapper"
      ).show();
...
//---------------------------------------------------
Demo.Skylink.on("dataChannelState", function(
    state,
    peerId,
    error,
    channelName,
...
Demo.Skylink.on("channelError", function(error) {
  Demo.Methods.displayChatMessage(
      "System",
      "Channel Error:<br>" + (error.message || error)
  );
});
//---------------------------------------------------
Demo.Skylink.on("recordingState", function(state, recordingId, url, error) {
  ...
  switch (state) {
    ...
    case Demo.Skylink.RECORDING_STATE.STOP:
      $("#recording_" + recordingId + "_state_icon").attr(
          "class",
          "glyphicon glyphicon-refresh"
      );
      $("#recording_" + recordingId + "_state").html(
          "STOPPED / PROCESSING VIDEO"
      );
      ...
      for (var prop in url) {
        if (url.hasOwnProperty(prop) && url[prop]) {
          $("#recording_" + recordingId + "_btn").append(
              ...
              'href="' +
              url[prop] +
              '" style="width:100%;margin:7px 0;display:block;" download="' +
              recordingId +
              "_" +
              prop +
              '.mp4">' +
              '<span class="glyphicon glyphicon-cloud-download"></span> <b>Download Recording (' +
              (prop !== "mixin" ? "Peer " : "") +
              prop +
              ")</b></a>"
          );
    ...
    case Demo.Skylink.RECORDING_STATE.ERROR:
      ...
      $("#recording_" + recordingId + "_error").html(
          "Recording session error:<br>" + (error.message || error.toString())
      );
  }
});

Demo.Skylink.on("getConnectionStatusStateChange", function(
    state,
    peerId,
    stats,
    error
  ...
  if (state === Demo.Skylink.GET_CONNECTION_STATUS_STATE.RETRIEVE_SUCCESS) {
    var statsElm = $("#video" + (peerId === "MCU" ? _peerId : peerId)).find(
        ".connstats"
    );
    var formatStatItem = function(type, dir) {
      ...
      if (
          !stats[type][dir].packetsLost ||
          stats[type][dir].packetsLost == "undefined"
      ) {
      ...
      itemStr +=
          "<br>Packets - (" +
          stats[type][dir].packets +
          " sent, " +
          stats[type][dir].packetsLost +
          " lost, " +
          stats[type][dir].jitter +
          " jitter" +
          (typeof stats[type][dir].jitterBufferMs === "number"
              ? ", " + stats[type][dir].jitterBufferMs + " jitter buffer <i>ms</i>"
              : "") +
          (dir === "sending" ? ", " + stats[type][dir].rtt + " rtt" : "") +
          (typeof stats[type][dir].nacks === "number"
              ? ", " + stats[type][dir].nacks + " nacks"
              : "") +
          (typeof stats[type][dir].plis === "number"
              ? ", " + stats[type][dir].plis + " plis"
              : "") +
          (typeof stats[type][dir].firs === "number"
              ? ", " + stats[type][dir].firs + " firs"
              : "") +
          (typeof stats[type][dir].slis === "number"
              ? ", " + stats[type][dir].slis + " slis"
              : "") +
          (typeof stats[type][dir].e2eDelay === "number"
              ? ", " + stats[type][dir].e2eDelay + " e2eDelay"
              : "") +
          ")";
      ...
      if (stats[type][dir].codec) {
        itemStr +=
            "<br>Codec - (name: " +
            stats[type][dir].codec.name +
            ", payload type: " +
            stats[type][dir].codec.payloadType +
            (stats[type][dir].codec.implementation
                ? ", impl: " + stats[type][dir].codec.implementation
                : "") +
            (stats[type][dir].codec.clockRate
                ? ", clockrate: " + stats[type][dir].codec.clockRate
                : "") +
            (stats[type][dir].codec.channels
                ? ", channels: " + stats[type][dir].codec.channels
                : "") +
            (stats[type][dir].codec.params
      ...
      if (type === "audio") {
        ...
        if (typeof stats.audio[dir].echoReturnLoss === "number") {
          itemAddStr +=
              (itemAddStr ? ", " : "") +
              "echo return loss: " +
              stats.audio[dir].echoReturnLoss;
        }
        ...
        if (typeof stats.audio[dir].echoReturnLossEnhancement === "number") {
          itemAddStr +=
              (itemAddStr ? ", " : "") +
              "echo return loss: " +
              stats.audio[dir].echoReturnLossEnhancement;
        }
      } else {
        itemStr += "<br>Frame - (";
        ...
        if (typeof stats.video[dir].frameWidth === "number") {
          itemAddStr +=
              (itemAddStr ? ", " : "") + "width: " + stats.video[dir].frameWidth;
        }
        ...
        if (typeof stats.video[dir].frameHeight === "number") {
          itemAddStr +=
              (itemAddStr ? ", " : "") +
              "height: " +
              stats.video[dir].frameHeight;
        }

        if (typeof stats.video[dir].frames === "number") {
          itemAddStr +=
              (itemAddStr ? ", " : "") + dir + ": " + stats.video[dir].frames;
        }

        if (typeof stats.video[dir].framesInput === "number") {
          itemAddStr +=
              (itemAddStr ? ", " : "") + "input: " + stats.video[dir].framesInput;
        }
        ...
        if (typeof stats.video[dir].frameRateMean === "number") {
          itemAddStr +=
              (itemAddStr ? ", " : "") +
              "fps mean: " +
              stats.video[dir].frameRateMean.toFixed(2);
        }
        ...
        if (typeof stats.video[dir].frameRateStdDev === "number") {
          itemAddStr +=
              (itemAddStr ? ", " : "") +
              "fps std dev: " +
              stats.video[dir].frameRateStdDev.toFixed(2);
        }
        ...
        if (typeof stats.video[dir].framesDecoded === "number") {
          itemAddStr +=
              (itemAddStr ? ", " : "") +
              "decoded: " +
              stats.video[dir].framesDecoded.toFixed(2);
        }
        ...
        if (typeof stats.video[dir].framesCorrupted === "number") {
          itemAddStr +=
              (itemAddStr ? ", " : "") +
              "corrupted: " +
              stats.video[dir].framesCorrupted.toFixed(2);
        }
        ...
        if (typeof stats.video[dir].framesPerSecond === "number") {
          itemAddStr +=
              (itemAddStr ? ", " : "") +
              "fps: " +
              stats.video[dir].framesPerSecond.toFixed(2);
        }
      }
    ...
    var formatCanStatItem = function(type) {
      $(statsElm)
          .find(".candidate ." + type)
          .html(
              (stats.selectedCandidate[type].ipAddress || "-") +
              ":" +
              (stats.selectedCandidate[type].portNumber || "-") +
              " - (transport: " +
              (stats.selectedCandidate[type].transport || "N/A") +
              ", type: " +
              (stats.selectedCandidate[type].candidateType || "N/A") +
              (stats.selectedCandidate[type].turnMediaTransport
                  ? ", turn media transport: " +
                  stats.selectedCandidate[type].turnMediaTransport
                  : "") +
              ")"
          );
    ...
    $(statsElm)
        ...
        .html(
            "Certificate algorithm - (local: " +
            (stats.certificate.local.fingerprintAlgorithm || "-") +
            ", remote: " +
    ...
    $(statsElm)
        ...
        .html(
            "Ciphers - (srtp: " +
            (stats.certificate.srtpCipher
                ? "<small>" + stats.certificate.srtpCipher + "</small>"
                : "N/A") +
            ", dtls: " +
            (stats.certificate.dtlsCipher
                ? "<small>" + stats.certificate.dtlsCipher + "</small>"
                : "N/A") +
            ")"
        );
  }
  ...
  window.selectTargetPeer = function(dom) {
    ...
    if (!dom.checked) {
      ...
      $(panelDom)
          .find(".selected-users")
          .append(
              '<em id="' + peerId + '-selected-user">Peer ' + peerId + "</em>"
          );
      $(panelDom)
          .find(".all")
          .hide();
  ...
  if (window.webrtcDetectedBrowser === "IE") {
    $("#file_list_panel").on("click", ".downloadfile", function() {
      ...
      window.navigator.msSaveBlob(
          Demo.Downloads[fileid].blob,
          Demo.Downloads[fileid].name
      );
    });
  }

  if (window.webrtcDetectedBrowser === "safari") {
    $("#file_list_panel .panel-heading").html(
        $("#file_list_panel .panel-heading").html() +
