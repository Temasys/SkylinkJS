...
Demo.Skylink.on('peerJoined', function(peerId, peerInfo, isSelf) {
  ...
  if ($('#video' + peerId).length === 0) {
    ...
    $(peerElm).append('<div class="connstats-wrapper"><button class="toggle-connstats btn-warning" data="' + (isSelf ? 'MCU' : peerId) +
      '">See ' + (isSelf ? 'MCU ' : '') + 'Stats</button><br/><br/><div class="row connstats">' +
