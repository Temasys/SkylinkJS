...
$(document).ready(function () {
  ...
  $('#get_btn').click(function () {
    ...
    if ($('#show_all').prop('checked')){
      Demo.Skylink.getPeers(true, function(error, success){
        if (error){
          alert('An error occurred while retrieving peer list',error);
        }
        else{

          var timestamp = new Date();
          $('#chat_log').append(Demo.Methods.displayChatItemHTML(displayName, timestamp, JSON.stringify(success), true));
          $('#chat_body').animate({
            scrollTop: $('#chat_body').get(0).scrollHeight
          }, 500);
      ...
      Demo.Skylink.getPeers(function(error, success){
        if (error){
          alert('An error occurred while retrieving peer list',error);
        }
        else{
