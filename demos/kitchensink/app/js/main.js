...
import Skylink, { SkylinkLogger, SkylinkEventManager, SkylinkConstants } from '../../../../dist/skylink.complete.js';
...
Demo.Methods.updateStreams = function() {
  Demo.Streams = Demo.Skylink.retrieveStreams(config.defaultRoom) || [];
};
...
SkylinkEventManager.addEventListener(SkylinkConstants.EVENTS.CHANNEL_REOPEN, (evt) => {
  ...
  .then(() => Demo.Skylink.joinRoom(joinRoomOptions))
});
...
SkylinkEventManager.addEventListener(SkylinkConstants.EVENTS.INCOMING_SCREEN_STREAM, (evt) => {
  const eventDetail = evt.detail;
...
SkylinkEventManager.addEventListener(SkylinkConstants.EVENTS.GET_CONNECTION_STATUS_STATE_CHANGE, (evt) => {
  ...
  if (state === SkylinkConstants.GET_CONNECTION_STATUS_STATE.RETRIEVE_SUCCESS) {
    ...
    var formatCanStatItem = function (type) {
      $(statsElm)
        .find('.candidate .' + type)
        .html((stats.selectedCandidate[type].ipAddress || '-') + ':' +
